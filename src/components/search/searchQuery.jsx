...
export default function SearchQuery({ query, closeModal }) {
  ...
  useEffect(() => {
    ...
    const otherRelated = results
      .filter((item) => item.score > score && item.score < 0.4)
      .map((res) => res.item);

    setQueryResults(queriedRes);
    setRelations(otherRelated)(results);
  }, [search, serviceData]);
  ...
  return (
    ...
    >
      ...
      <Container>
        <div className="close__search__modal" onClick={closeModal}>
          {' '}
          <Xmark strokeWidth={1} />{' '}
        </div>
        ...
        <div className="search__title">
          ...
          >
            <span>Hey there,</span> get curated information from all news, services & topics{' '}
          </motion.div>
        </div>
        ...
        <div className="search__query__results">
          {isLoading ? (
            ...
            <p className="feedback"> Start searching and get results by typing or speaking...</p>
          ) : (
            <p className="feedback">
              {' '}
              No results found for <span style={{ color: 'green' }}>'{search}'</span> at the moment,
          ...
          {relations.length ? (
            <div className="relations">
              <div className="relations__title">
                {' '}
                - Other Related Services - {relations?.length} Results{' '}
              </div>
        ...
        <div className="search__field flex">
          <div className="useCard">
            ...
            {search !== '' ? (
              <div className="closeInput" onClick={() => setSearch('')}>
                {' '}
                <Xmark color="#fff" />{' '}
              </div>
            ) : null}
