...
export default function Services({ bgColor, location, data_limit }) {
  ...
  return (
    <div
      className={`services ${location === 'services' ? 'server' : ''}`}
      id="services"
    ...
    >
      <Container>
        {location === 'home' ? (
          <div className="midTopic">
            ...
            <div className="subtitle uppercase">
              Making it easier to have quick access to Lagos State services and information
            </div>
          </div>
        ) : null}
        ...
        {!categoryData.isLoading ? (
          <div
            className={`servicesPlatoon ${location === 'services' ? 'servicePagePlatoon' : ''} `}
          >
            {categories
              ?.filter((c) => !c.isOffline)
              ?.map((lasg_service, index) => {
                return (
                  <motion.div
                    className="services_card"
                    onClick={() =>
                      (window.location.href = `/services/${lasg_service.formattedName}`)
                    }
                    key={index}
                  >
                    <div className="icon">
