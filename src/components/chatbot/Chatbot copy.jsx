...
const CHAT_MODEL_NAME = 'gemini-3.1-flash-lite-preview';

// FIX: Use explicit 'models/' prefix to resolve 404 on v1beta endpoint.
// This keeps compatibility with existing Supabase vector(768) columns.
// If you migrate to gemini-embedding-001, update your Supabase column to vector(3072).
...
`;
  ...
  const getRelevantLagosContext = useCallback(
    async (query) => {
      try {
        // FIX: Using EMBEDDING_MODEL_NAME constant with explicit 'models/' prefix
  ...
  const handleSubmit = useCallback(
    async (customInput) => {
      ...
      // FIX: Was referencing undefined MODEL_NAME — now correctly uses CHAT_MODEL_NAME
