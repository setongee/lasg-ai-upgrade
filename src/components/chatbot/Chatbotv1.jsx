...
// ---- SINGLE Gemini Instance (CRITICAL FIX) ----
...
const Chatbot = ({ pageContext }) => {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content: `Hello! **${getTimeOfDayGreeting()}** I'm **Seth** - your AI assistant from the Lagos State Ministry of Innovation, Science and Technology.`,
    },
  ]);
  ...
  // ---- CRITICAL: Single chat session per component lifecycle ----
  const chatSessionRef = useRef(null);
  const location = useLocation();

  // ---- Memoize ministry context to prevent recreating on every render ----
  const ministryContext = useMemo(() => {
    const history = messages
      .slice(-6) // Only use last 6 messages for context (cost optimization)
...
🧭 Behavioural Rules:
- Always interpret questions like "Who is the commissioner?", "What are their services?", etc. as referring to **${
      ministryInfo ? ministryInfo.name : 'the relevant Lagos State Ministry'
    }**.
...
${
  ministryInfo
    ? `${ministryInfo.name} (${ministryInfo.url})`
    : 'Automatically infer from page context'
}

📚 Recent Conversation:
${history}
`;
  }, [safePage, ministryInfo, messages]);
  ...
  }, [isChatOpen, scrollToBottom, initializeChatSession]);

  // ---- Reset chat when navigating to different page ----
  useEffect(() => {
    setIsChatOpen(false);
    chatSessionRef.current = null; // Reset session on page change
    suggestionCache.clear(); // Clear cache
    document.body.style.overflow = 'auto';
  }, [location]);
  ...
  // ---- Generate initial suggestions with caching ----
  useEffect(() => {
    async function generateInitialSuggestions() {
      ...
      if (isGeneralContext) {
        if (hour >= 5 && hour < 12) {
          timeContext = 'Morning — focus on commute, breakfast, early services.';
        } else if (hour >= 12 && hour < 17) {
          timeContext = 'Afternoon — focus on lunch, office hours, errands.';
        } else if (hour >= 17 && hour < 22) {
          timeContext = 'Evening — focus on safety, traffic, nightlife.';
        } else {
          timeContext = 'Late Night — focus on emergencies, 24-hour services.';
        }
      }

      const prompt = `Generate 6 short questions (under 10 words each) Lagos citizens might ask about "${contextName}". ${timeContext} Format: question1||question2||question3||question4||question5||question6`;
    ...
    async (query) => {
      try {
        ...
        const { data, error } = await supabase.rpc('match_lagos_services', {
          ...
          match_threshold: 0.7,
          match_count: 2, // Reduced from 3 to save processing
        });
  ...
  // ---- Generate follow-ups with reduced frequency ----
  const generateFollowUps = useCallback(async (lastReply, sender) => {
    // Only generate follow-ups for assistant responses, not every message
  ...
  // ---- Simplified location detection (avoid extra API call) ----
  ...
  const handleSubmit = useCallback(
    async (customInput) => {
      ...
      try {
        ...
        // Generate follow-ups only occasionally (not every time)
  ...
  return (
    <>
      <div className="bubblePage" onClick={() => setIsChatOpen(true)}>
        ...
        <div className="chatParent">
          <div className="chat-title">
            ...
            <div className="identity flex justify-between items-center">
              ...
              <div className="closeChatModal" onClick={() => setIsChatOpen(false)}>
                <Xmark strokeWidth={2} />
              </div>
            </div>
          </div>
