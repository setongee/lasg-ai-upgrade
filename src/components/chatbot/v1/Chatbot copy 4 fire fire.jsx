...
const Chatbot = ({ pageContext }) => {
  ...
  const [userLga, setUserLga] = useState(null);
...
`;
  ...
  // Detect location issue
  async function detectNeedsLocationAI(message) {
    const prompt = `
    You are a classification model. Does the following message require a physical location to answer correctly? 
  ...
  **${ministryInfo ? ministryInfo.name : 'the appropriate Lagos State Ministry or Department'}**
  ...
  `;
      ...
      for await (const chunk of stream) {
        ...
        // ✅ Always keep arrow visible while streaming
        setShowScrollButton(true);
      }

      // ✅ After streaming: keep arrow visible (do NOT hide it automatically)
