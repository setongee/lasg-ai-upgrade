...
const UpcomingEventsEdit = () => {
  const { mdaEditData, setMdaEditData } = useEditDataStore();
  const isEnabled = mdaEditData.enabledSections?.upcomingEvents !== false;
  const backgroundColor = mdaEditData.upcomingEvents?.backgroundColor;
  ...
  return (
    <div className="fixed top-[145px] left-0 w-[350px] h-[calc(100vh-145px)] bg-white p-[30px] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] z-90">
      ...
      <p className="text-[13px] text-gray-500 leading-[1.6] mt-6">
        This section automatically shows your 3 most recent events, and disappears on its own if
        you haven't created any yet. To add or manage events, go to{' '}
        <span className="font-semibold text-gray-700">Events</span> in the sidebar.
      </p>
    </div>
  );
};
