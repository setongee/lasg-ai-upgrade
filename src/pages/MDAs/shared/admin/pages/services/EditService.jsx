...
import { formattedName, isExistingService } from '../../../../api/admin/logic';
...
const EditService = ({ selectView, service, setShowEditServiceItem, services, refetch }) => {
  const [data, setData] = useState({
    ...service,
    categories:
      service.categories?.map((cat) => (typeof cat === 'string' ? { name: cat, _id: cat } : cat)) ||
      [],
    customKeywords: service.customKeywords || [],
  });
  ...
  const handleCheck = (item) => {
    const existingService = isExistingService(services?.data, item.name);
  ...
  const handleToggleCategory = (category) => {
    // Prevent toggling ministry category
    if (category.name === mda.fullname) {
      return;
    }
  ...
  const removeCategory = (categoryId) => {
    // Find the category to check if it's the ministry category
    const categoryToRemove = data.categories.find(
      (c) => c._id === categoryId || c.name === categoryId
    );

    // Prevent removing ministry category
    if (categoryToRemove?.name === mda.fullname) {
      return;
    }
  ...
  const displayCategories = data.categories?.filter((cat) => cat.name !== mda.fullname) || [];
  const hasMinistryCategory = data.categories?.some((cat) => cat.name === mda.fullname) || false;
  ...
  const handleSubmit = (e) => {
    ...
    if (data.name !== service?.name && check) {
    ...
    updateSingleService(data._id, submissionData).then((res) => {
          ...
          <div className="flex flex-col gap-1.5 relative" ref={dropdownRef}>
            ...
            >
              <div className="flex flex-wrap gap-2 items-center flex-1">
                ...
                {/* Show ministry category silently if it exists */}
                {hasMinistryCategory && <span className="hidden">Ministry category present</span>}
              </div>
              <NavArrowDown
                height={18}
                width={18}
              ...
              <div className="absolute top-[100%] left-0 right-0 z-50 mt-1 bg-white ring-1 ring-gray-200 rounded-[4px] shadow-lg max-h-[300px] flex flex-col">
                ...
                <div className="overflow-y-auto flex-1">
                  {categoriesLoading ? (
                    ...
                    filteredCategories
                      .filter((category) => category.name !== mda.fullname) // Exclude ministry category from dropdown
                      .map((category) => {
                        const isSelected = data.categories?.some((c) => c.name === category.name);
                        return (
                          <div
                            key={category._id}
                          ...
                          >
                            ...
                            >
                              {isSelected && (
                                <Check height={14} width={14} className="text-white" />
                              )}
                            </div>
                          </div>
                        );
